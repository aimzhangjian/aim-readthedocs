...
def serve_docs(request, project, subproject,
    ...
    try:
        if request.META['HTTP_REFERER'].count("/") == 3:
            project.reading_amount = project.reading_amount + 1
        elif request.META['HTTP_REFERER'].count("/") == 5:
            project.reading_amount = project.reading_amount + 1
        project.save()
    except Exception:
        project.reading_amount = project.reading_amount + 1
...
def _serve_symlink_docs(request, project, privacy_level, filename=''):
    ...
    if (settings.DEBUG or constants.PUBLIC in serve_docs) and privacy_level != constants.PRIVATE:
        ...
        if os.path.exists(os.path.join(basepath, filename)):
            files_name=os.path.join(basepath, filename)
            if files_name.endswith('.txt'):
                with open(files_name,'r') as f:
                    s = f.read()
                    with open(files_name,'w') as f1:
                        s1 = s.decode('utf-8', 'ignore').encode('utf-8_sig')
                        f1.write(s1)
